...
public class ClientController implements Runnable {
	...
	private void getMessage() throws IOException {
		headerData = new byte[ProtocolProperties.HEADER_LENGTH];
	...
	public void sendOnePLNCoinsCountRequest() throws IOException {
		...
		++keepAliveCounter;
		FilesOperations.saveDataToFile(System.currentTimeMillis()
				+ "   Grupa: CYKLICZNE_DANE_FINANSOWE  " + data[0] + data[1]
				+ "\n");
		clientModel.setOnePLNCoins(data[0]);
		
		sendMessage(new byte[]{ (byte) 0xFF, (byte) 0xFE, (byte) 0x00, (byte) 0x01 });
	}
